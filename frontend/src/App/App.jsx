...
const api = {
  ...
  // Разбить текст на задачи через AI
  splitTextToTasks: async (text) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(error.detail || 'Ошибка обработки текста');
    }
    
    return response.json();
  },

  // Получить энергию (выполненные задачи)
  getEnergy: async () => {
    const response = await fetch(`${API_BASE_URL}/energy`);
    if (!response.ok) throw new Error('Ошибка загрузки энергии');
    const data = await response.json();
    return {
      energy: data.energy || 0,
      message: data.message || `Выполнено ${data.energy || 0} задач`
    };
  },
